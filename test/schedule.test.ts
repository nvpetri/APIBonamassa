...
test("daytime shift and midnight/month/year rollover", () => {
  assert.equal(
    operation(
      { ...store, opensAt: "09:00", closesAt: "18:00" },
      new Date("2026-09-14T12:00:00-03:00"),
    ).open,
    true,
  );
  assert.equal(
    operation(store, new Date("2026-12-31T23:59:00-03:00")).nextOpening,
    "2027-01-01T20:00:00.000Z",
  );
  assert.equal(
    operation(
      { ...store, opensAt: "00:00", closesAt: "04:00" },
      new Date("2026-09-14T03:59:00-03:00"),
    ).open,
    true,
  );
});
test("temporary override expires exactly at the scheduled boundary", () => {
  const early = {
    ...store,
    overrideOpen: true,
    overrideUntil: new Date("2026-09-14T17:00:00-03:00"),
  };
  assert.equal(
    operation(early, new Date("2026-09-14T15:00:00-03:00")).open,
    true,
  );
  ...
  assert.equal(
    operation(early, new Date("2026-09-15T03:00:00-03:00")).open,
    false,
  );
});
test("manual pause does not permanently override tomorrow's opening", () => {
  const paused = {
    ...store,
    overrideOpen: false,
    overrideUntil: new Date("2026-09-15T03:00:00-03:00"),
  };
  assert.equal(
    operation(paused, new Date("2026-09-14T20:00:00-03:00")).open,
    false,
  );
  assert.equal(
    operation(paused, new Date("2026-09-15T17:00:00-03:00")).open,
    true,
  );
});
test("legacy manual mode does not promise reservations without a schedule", () => {
  const state = operation(
    ...
    new Date("2026-09-14T20:00:00-03:00"),
  );
