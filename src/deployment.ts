...
export function deployment(env: NodeJS.ProcessEnv = process.env) {
  ...
  for (const entry of trustedProxies) {
    ...
    if (!family || extra !== undefined || (prefix !== undefined && (!/^\d+$/.test(prefix) || Number(prefix) < 1 || Number(prefix) > (family === 4 ? 32 : 128))))
      throw new Error("TRUSTED_PROXY_CIDRS aceita somente IPs/CIDRs explícitos; não use true, número de saltos ou rede /0.");
  }
  if (strict) {
    if (env.NODE_ENV !== "production") throw new Error("Staging/produção requer NODE_ENV=production.");
    let database: URL;
    ...
    if (!["postgres:", "postgresql:"].includes(database.protocol) || !database.hostname ||
      ...
      throw new Error("Banco de staging/produção requer PostgreSQL com sslmode=require e sslaccept=strict.");
    if (env.DOCS_ENABLED === "true") throw new Error("Desative DOCS_ENABLED em staging/produção.");
    if (env.SEED_DEMO === "true") throw new Error("SEED_DEMO não é permitido em staging/produção.");
  }
