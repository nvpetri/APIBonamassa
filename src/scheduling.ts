...
export class SchedulingService implements OnModuleInit, OnModuleDestroy {
  ...
  async sync(tx: Tx, store: Store, now = new Date()) {
    ...
    if (state.open) {
      ...
      const due =
        state.overrideOpen === true && !state.scheduledOpen && state.nextOpening
          ? new Date(state.nextOpening)
          : now;
      const orders = await tx.order.findMany({
        where: {
          storeId: store.id,
          status: "SCHEDULED",
          scheduledFor: { lte: due },
        },
        select: {
          id: true,
          version: true,
      ...
      if (orders.length) {
        ...
        await tx.orderEvent.createMany({
          data: orders.map((o) => ({
            orderId: o.id,
            version: o.version + 1,
            action: "schedule-released",
            actorId: null,
            data: { scheduledFor: o.scheduledFor!.toISOString() },
            createdAt: now,
          })),
        });
        events.push(
