...
export class Mailer {
  async code(to: string, code: string, kind: "verify" | "reset") {
    const env = config();
    const subject = kind === "verify" ? "Confirme seu e-mail na Bonamassa" : "Código para redefinir sua senha";
    const action = kind === "verify" ? "confirmar seu e-mail" : "redefinir sua senha";
    if (!env.EMAIL_API_KEY) {
      if (env.NODE_ENV === "production") throw new Error("EMAIL_API_KEY não configurada.");
