...
`POST /sessions` recebe `{"storeSlug":"bonamassa","email":"...","password":"..."}` e devolve `accessToken`, `tokenType: "Bearer"`, `expiresAt` e `user`. O token é opaco, aleatório; somente seu SHA-256 fica no PostgreSQL. A sessão expira após **cinco dias sem uso**, definidos por `SESSION_IDLE_DAYS=5`. Cada requisição autenticada renova a janela; uso diário mantém a sessão, sem prazo absoluto contado desde o login. A variável antiga `SESSION_HOURS` não controla mais a validade. As respostas privadas fornecem `X-Session-Expires-At`, que o painel usa para renovar também o cookie HttpOnly. Os Androids mantêm o token e tratam 401 da API. `DELETE /sessions/current` revoga a sessão. `POST /me/password` confere a senha atual e revoga todas as sessões, inclusive a atual.

O cadastro responde `verificationRequired`, sem token. `POST /auth/email-verification/request` solicita um código e `POST /auth/email-verification/confirm` recebe `storeSlug`, `email` e `code`. A confirmação válida emite a primeira sessão. Contas já confirmadas devem entrar com senha: nenhum código, inclusive um já utilizado, permite emitir outra sessão. Códigos têm validade de dez minutos por padrão, até cinco tentativas e uso único; reenvio invalida os anteriores. `POST /auth/password-reset/request` e `/confirm` implementam recuperação; a confirmação recebe também `newPassword` e revoga todas as sessões antigas. O código fixo existe apenas com `NODE_ENV=test`, em banco isolado; em produção o envio exige `EMAIL_API_KEY` e remetente configurado no provedor.
